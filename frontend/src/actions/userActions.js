...
export const loginUser = (email, password) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(
			'https://food-bank.vercel.app/users/login',
			{ email, password },
			config
		);
...
export const refreshLogin = (email) => async (dispatch, getState) => {
	try {
		...
		if (userInfo.isSocialLogin) {
			...
			const { data } = await axios.post(
				'https://food-bank.vercel.app/users/refresh',
				{
					email,
...
export const registerUser = (name, email, password) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(
			'https://food-bank.vercel.app/users/',
			{ name, email, password },
			config
		);
...
export const sendVerficationEmail = (email) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(
			'https://food-bank.vercel.app/users/confirm',
			{ email },
			config
		);
	...
	async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.get(
				`https://food-bank.vercel.app/users/confirm/${emailToken}`
			);
	...
	(passwordToken, password) => async (dispatch) => {
		try {
			...
			const { data } = await axios.put(
				'https://food-bank.vercel.app/users/reset',
	...
	try {
		...
		if (userInfo.isSocialLogin) {
			...
			let { data } = await axios.post(
				'https://food-bank.vercel.app/users/passport/data/',
				{ id },
				config
			);
			dispatch({
			...
			const { data } = await axios.get(`https://food-bank.vercel.app/users/${id}`, config);
			dispatch({
				type: USER_DETAILS_SUCCESS,
				payload: { ...data, isSocialLogin: false },
			});
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.put('https://food-bank.vercel.app/users/profile', user, config);
...
export const listAllUsers =
	...
	async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.get(
				`https://food-bank.vercel.app/users?pageNumber=${pageNumber}`,
				config
			);
...
export const deleteUser = (id) => async (dispatch, getState) => {
	try {
		...
		await axios.delete(`https://food-bank.vercel.app/users/${id}`, config);
...
export const updateUser = (user) => async (dispatch, getState) => {
	try {
		...
		const isSocial = userInfo.isSocialLogin;
		const { data } = await axios.put(
			`https://food-bank.vercel.app/users/${user._id}`,
			user,
			config
		);
